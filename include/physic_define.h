...
#define SUBPIXEL_MULTIPLIER (1 << SUBPIXEL_BITS) // 256
#define PIXELS_PER_BLOCK 16
#define BLOCKS_TO_VALUE (PIXELS_PER_BLOCK * SUBPIXEL_MULTIPLIER) // 4096
...
#define CUBE_FIRST_JUMP_SPEED    sqrtf(2 * CUBE_GRAVITY * CUBE_FIRST_JUMP_HEIGHT)

#define CUBE_JUMP_HEIGHT         (2.1653 * BLOCKS_TO_VALUE)
#define CUBE_JUMP_SPEED          sqrtf(2 * CUBE_GRAVITY * CUBE_JUMP_HEIGHT)

#define CUBE_MINI_JUMP_SPEED     (CUBE_JUMP_SPEED * 0.8)

#define CUBE_YELLOW_PAD_MAX_HEIGHT (4.2 * BLOCKS_TO_VALUE)
#define CUBE_YELLOW_PAD_JUMP_SPEED sqrtf(2 * CUBE_GRAVITY * CUBE_YELLOW_PAD_MAX_HEIGHT)
...
#define SHIP_YELLOW_PAD_JUMP_SPEED sqrtf(2 * SHIP_GRAVITY_AFTER_HOLD * SHIP_YELLOW_PAD_MAX_HEIGHT)
...
static Physics getPhysics(VehicleType vehicle, int isMini) {
    ...
    if (vehicle == VEHICLE_CUBE) {
        ...
        float jumpSpeed = sqrtf(2 * (gravityBase * 1.2) * SHIP_YELLOW_PAD_MAX_HEIGHT);
